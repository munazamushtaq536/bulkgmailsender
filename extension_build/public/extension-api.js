...
const CONFIG = {
    ...
    VERCEL_URL: 'https://bulkgmailsenderam.vercel.app', // Update with correct URL
    API_MOCK_ENABLED: true  // Keep true while user fixes Vercel env vars
};
...
window.fetch = async function (url, options = {}) {
    ...
    if (CONFIG.USE_VERCEL && typeof url === 'string' && url.includes('/api/')) {
        ...
        try {
            const response = await originalFetch(remoteUrl, options);
            if (!response.ok) {
                console.warn(`[Extension API] Vercel returned status ${response.status} for ${remoteUrl}`);
            }
            return response;
        } catch (err) {
            console.error('[Extension API] Vercel Fetch Failed:', err);
            // Fallback to mock if Vercel is completely unreachable/down
            if (CONFIG.API_MOCK_ENABLED) {
                console.info('[Extension API] Falling back to Mock API...');
...
async function checkBackendHealth() {
    if (!CONFIG.USE_VERCEL) return { online: true, mode: 'local' };
    try {
        const res = await originalFetch(CONFIG.VERCEL_URL + '/api/check-auth');
        const data = await res.json();
        return { online: true, mode: 'vercel', ...data };
    } catch (err) {
        return { online: false, mode: 'vercel', error: err.message };
    }
}
